...
import TEST_HOMEPAGE_LAYOUT from './queries/test_homepage_layout';
...
function TestHomePageLayout({ render, }: { render: Function, }): React.Node {
  return (
    <Query query={TEST_HOMEPAGE_LAYOUT}>
      {({ loading, error, data, client, }) => {
        if (loading) return null;
        if (error) return console.error(error);
        const {
          testPage: { slots, seoData, pageDateTimeString, pageType, globalLazyload, },
        } = data;
...
export default TestHomePageLayout;
