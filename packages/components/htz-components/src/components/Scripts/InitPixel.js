...
export default class InitPixel extends Component {
  ...
  facebookPixel = hostname => {
    ...
    const facebookPixelId = hostname.includes('themarker.com')
      ? 'update this'
      : '801998859871552';
    return (
      <React.Fragment>
  ...
  googlePixel = hostname => {
    // todo: update this if we use in other sites
    const googlePixelId = hostname.includes('themarker.com')
      ? 'update this'
      : 'AW-955543703';
    return (
      <React.Fragment>
  ...
  chromePush = hostname => {
    // todo: update this if we use in other sites
    const applicationCode = hostname.includes('themarker.com')
      ? 'update this'
      : '3624D-2BB40';
    const safariWebsitePushID = hostname.includes('themarker.com')
      ? 'update this'
      : 'web.com.haaretz.webpush';
    const defaultNotificationTitle = hostname.includes('themarker.com')
      ? 'TheMarker'
      : 'הארץ';
